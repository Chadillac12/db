# Requirements Interactive Viewer (DataviewJS)

Paste the block below into an Obsidian note.
...
try {
  ...
  // Bump this if you change filter schema
  const FILTER_STATE_KEY = "requirements_filter_state_grouped_sectiontitle_v5";
  ...
  const STYLES = {
    ...
    helpText: { fontSize: "0.8em", color: "var(--text-muted)" },
    ...
    fallbackPre: { whiteSpace: "pre-wrap", margin: "0" },
    imgGrid: {
      marginTop: "10px",
      display: "flex",
      flexWrap: "wrap",
  ...
  function applySavedSelection(selectEl, values) {
  ...
  function compareSectionIds(a, b) {
    ...
    // Put "no numeric id" AFTER numbered sections
    if (!as && bs) return 1;
    if (as && !bs) return -1;
    if (!as && !bs) return 0;

    const aParts = as.split(".").map(x => parseInt(x, 10) || 0);
    const bParts = bs.split(".").map(x => parseInt(x, 10) || 0);
  ...
  function resolveVaultLinkToFile(linkText, sourcePath) {
    ...
    // Resolve via metadata cache (handles attachments by name)
    return app.metadataCache?.getFirstLinkpathDest?.(cleaned, sourcePath) || null;
  }
  ...
  function renderLiteMarkdown(targetEl, markdown, sourcePath) {
    // Minimal renderer for headings, paragraphs, bullet lists, pipe tables, and embedded images.
    targetEl.innerHTML = "";
    const lines = String(markdown || "").split("\n");

    const imgGrid = targetEl.createEl("div");
    Object.assign(imgGrid.style, STYLES.imgGrid);

    function addImagesFromText(txt) {
      // Obsidian embeds: ![[...]]
      const wikiEmbed = /!\[\[([^\]]+)\]\]/g;
      let m;
      while ((m = wikiEmbed.exec(txt)) !== null) {
        const file = resolveVaultLinkToFile(m[1], sourcePath);
        if (file && isImageFile(file)) {
          const img = imgGrid.createEl("img");
    ...
    while (i < lines.length) {
      ...
      // table
      const t = parseMarkdownTable(lines, i);
      if (t) {
        const table = targetEl.createEl("table");
        ...
        // also scan table text for embeds
        addImagesFromText([t.headers.join(" "), ...t.rows.map(rr => rr.join(" "))].join("\n"));
      ...
      if (h3) { targetEl.createEl("h3", { text: h3[1] }); i++; continue; }
      if (h4) { targetEl.createEl("h4", { text: h4[1] }); i++; continue; }

      // bullets
      const bullet = line.match(/^\s*[-*]\s+(.*)$/);
      if (bullet) {
        const ul = targetEl.createEl("ul");
        while (i < lines.length) {
          const b = lines[i].match(/^\s*[-*]\s+(.*)$/);
          if (!b) break;
          const li = ul.createEl("li");
          li.textContent = b[1];
          addImagesFromText(lines[i]);
          i++;
        }
        continue;
      }
      ...
      if (line.trim() !== "") {
        ...
        p.textContent = line;
        addImagesFromText(line);
      }
      i++;
    }

    // As a last resort, show raw text if we rendered nothing
    if (!targetEl.childElementCount) {
      const pre = targetEl.createEl("pre");
      Object.assign(pre.style, STYLES.fallbackPre);
      pre.textContent = markdown;
      addImagesFromText(markdown);
    }
  }
  ...
  async function renderMarkdownInto(targetEl, markdown, sourcePath) {
    ...
    renderLiteMarkdown(targetEl, markdown, sourcePath);
  }
  ...
  const createFilter = (label, type, options = [], opts = {}) => {
    ...
    if (type === "select") {
      ...
      if (opts.multiple) {
        sel.multiple = true;
        sel.size = Math.min(Math.max(5, Math.floor(options.length * 0.6)), 10);
        const help = group.createEl("div", { text: "Tip: Ctrl/⌘-click for multi-select" });
        Object.assign(help.style, STYLES.helpText);
      } else {
  ...
  const filters = {
    ...
    reqType: createFilter("Requirement Type", "select", reqTypes, { multiple: true }),
  ...
  for (const k in filters) {
    ...
    if (filters[k].type === "checkbox") filters[k].checked = Boolean(saved[k]);
    else if (filters[k].multiple) applySavedSelection(filters[k], saved[k]);
    else filters[k].value = saved[k];
  }
  ...
  async function filterResults() {
    ...
    if (filters.reqType.multiple) {
      const selected = Array.from(filters.reqType.selectedOptions || []).map(o => o.value).filter(Boolean);
      if (selected.length) {
        const sel = new Set(selected);
        filtered = filtered.filter(p => sel.has(p.Requirement_Type ?? p.Section_Type));
      }
    }
  ...
  function buildQueue(filteredPages) {
    ...
    // Group ordering (numeric section prefix top-to-bottom)
    ...
    // Within-group ordering (numeric where possible)
    ...
    return { queue, groups, groupsCount: groups.length };
  }
  ...
  function renderGroupHeader(group) {
    ...
    title.textContent = group.title; // Section_Title
  ...
  function compareSectionStrings(a, b) {
    ...
    return compareSectionIds(aKey || aRaw, bKey || bRaw) || String(a.Req_ID ?? a.file?.path ?? "").localeCompare(String(b.Req_ID ?? b.file?.path ?? ""));
  }

  // =========================
  // CANVAS EXPORT (group boxes + sim placement option)
  // =========================
  ...
  async function exportToCanvas(filteredPages) {
    try {
      ...
      // Heading extraction (H2 blocks)
      function extractH2Block(markdown, headingText) {
        if (!markdown) return "";
        const esc = headingText.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        const startRe = new RegExp(`^##\\s+${esc}\\s*$`, "mi");
        const m = markdown.match(startRe);
      ...
      async function estimateNodeSize(page, opts) {
        ...
        if (opts.kind === "core") {
          ...
          if (reqText) h += Math.floor(reqText.length / 900) * 120;
        }

        if (opts.layout === "whole" || opts.kind === "sim") {
          const md = await loadMd(page.file.path);
          const src = (opts.kind === "sim") ? (extractH2Block(md, opts.simHeading) || "") : md;
      ...
      const simScope = (await customPrompt("Simulink source scope? (none/self/parents/children/both)", "self") || "self")
        .trim().toLowerCase();
      const simHeading = (await customPrompt("Simulink heading text (exact H2)", CONFIG.HEADING_SIMULINK) || CONFIG.HEADING_SIMULINK).trim();

      // NEW: simulink placement
      const simPlacement = (await customPrompt("Simulink placement? (side/below)", "side") || "side").trim().toLowerCase();

      // Depth:
      // 0 = core only
      // 1 = parents + children
      // 2 = parents + grandparents + children + grandchildren
      ...
      // Canvas collections
      const nodes = [];
      const groupNodes = []; // Groups collected separately
      const edges = [];
      const added = new Set();
      ...
      function addEdge(fromId, toId, color = null, fromSide = "right", toSide = "left") {
        edges.push({
          id: `e_${fromId}_${toId}`,
          fromNode: fromId,
          fromSide,
      ...
      async function addFileNode(page, id, x, y, w, h, color, subpath) {
        ...
        added.add(id);
        const node = { id, type: "file", file: String(page.file.path), x, y, width: w, height: h, color };
      ...
      async function addCoreNode(page, x, y, color) {
        ...
        if (mode === "file") {
          const subpath = (layout === "req") ? `#${CONFIG.HEADING_REQ_TEXT}` : null;
          ...
          const body = [
            ...
            "",
            `[[${page.file.path}#${CONFIG.HEADING_REQ_TEXT}]]`,
      ...
      async function addRelatedReqNode(page, idSuffix, x, y, color) {
        ...
        if (mode === "file") {
          await addFileNode(page, id, x, y, w, h, color, `#${CONFIG.HEADING_REQ_TEXT}`);
        } else {
          const title = page.Section_Title || page.Section || page.Req_ID || page.file.name;
      ...
      async function hasSimulinkSection(page) {
        const md = await loadMd(page.file.path);
        const block = extractH2Block(md, simHeading);
        return Boolean(block && block.trim().length > 0);
      }

      async function addSimulinkNodesFor(corePage, coreNode, baseX, baseY, coreY) {
        if (simScope === "none") {
          return { rightEdge: coreNode.w, usedHeight: 0, bottomY: coreY + coreNode.h, hasAny: false };
        }
        ...
        const fromSide = simPlacement.startsWith("b") ? "bottom" : "right";
        const toSide = simPlacement.startsWith("b") ? "top" : "left";
        ...
        for (const c of shown) {
          ...
          addEdge(coreNode.id, simId, "2", fromSide, toSide);
        ...
        if (overflow.length > 0) {
          ...
          addEdge(coreNode.id, moreId, "2", fromSide, toSide);
          rightEdge = Math.max(rightEdge, baseX + w);
          y += h + GAP_Y * 0.5;
          usedHeight = y - baseY;
        }

        const bottomY = baseY + usedHeight;
        return { rightEdge, usedHeight, bottomY, hasAny: true };
      }

      // Grouping order: use the SAME grouping/sorting as the viewer
      const { groups } = buildQueue(filteredPages);

      new Notice(`🧱 Exporting ${filteredPages.length} requirements…`);
      ...
      for (const g of groups) {
        const groupId = safeId(g.title, "group");
        const groupTitle = g.title || "Unsectioned";
        const groupSubtitle = g.sortId ? `Section ${g.sortId}` : "";

        // Track group bounds
        let groupTop = yCursor - PADDING_Y;
        let groupBottom = yCursor;
        let groupMinX = -PADDING_X;   // core column starts at x=0
        let groupMaxX = PADDING_X;    // will expand based on core+sim widths

        // Add group node NOW; we'll update its width/height at end
        groupNodes.push({
          id: groupId,
          type: "group",
          x: groupMinX,
          ...
          width: 800,
          height: 200,
          label: groupSubtitle ? `${groupTitle}\n${groupSubtitle}` : groupTitle,
          color: "6"
        });
        ...
        for (const corePage of g.pages) {
          ...
          // simulink placement base
          let simBaseX = coreX + core.w + GAP_X;
          let simBaseY = coreY;

          if (simPlacement.startsWith("b")) {
            // below core
            simBaseX = coreX;
            simBaseY = coreY + core.h + GAP_Y * 0.5;
          }

          // simulink nodes
          const sim = await addSimulinkNodesFor(corePage, core, simBaseX, simBaseY, coreY);

          // Track row height
          let rowHeight = core.h;
          if (sim.hasAny) rowHeight = Math.max(rowHeight, sim.bottomY - coreY);

          // Expand group width to include core + sim column/stack
          groupMaxX = Math.max(groupMaxX, coreX + core.w);
          if (sim.hasAny) groupMaxX = Math.max(groupMaxX, sim.rightEdge);

          // -------- Parents / Grandparents (left) --------
          if (includeParents && depth >= 1) {
            ...
            for (const p of parents) {
              ...
              addEdge(pn.id, core.id, "4"); // parent -> core

              // grandparents (one more step left)
              if (depth >= 2) {
                ...
                for (const gp of gps) {
                  ...
                  addEdge(gpn.id, pn.id, "4"); // grandparent -> parent
                  gpY += gpn.h + GAP_Y * 0.4;
          ...
          // -------- Children / Grandchildren (right) --------
          if (includeChildren && depth >= 1) {
            const kids = getChildrenOf(corePage).slice(0, maxChildren);

            const childrenBaseX = sim.hasAny
              ? (Math.max(coreX + core.w, sim.rightEdge) + GAP_X)
              : (coreX + core.w + GAP_X);
            ...
            for (const c of kids) {
              ...
              addEdge(core.id, cn.id, "5"); // core -> child

              // grandchildren (one more step right)
              if (depth >= 2) {
                ...
                for (const gc of gcs) {
                  ...
                  addEdge(cn.id, gcn.id, "5"); // child -> grandchild
                  gcY += gcn.h + GAP_Y * 0.4;
        ...
        if (groupNode) {
          groupNode.width = (groupMaxX - groupMinX) + PADDING_X;
          groupNode.height = (groupBottom - groupTop) + PADDING_Y;
        }
      ...
      // Merging: Groups FIRST (drawn at back), then Content Nodes (drawn on top)
      const allNodes = [...groupNodes, ...nodes];

      const jsonStr = JSON.stringify({ nodes: allNodes, edges }, null, 2);
      await writeOrModify(fullPath, jsonStr);
  ...
  renderAllPdfBtn.onclick = async () => {
    try {
      ...
      // Keep calling loadMore until everything is in the DOM
      let safety = 0;
      while (displayedCount < currentQueue.length) {
        await loadMore();
        safety++;
        if (safety % 3 === 0) await uiYield(); // yield every few batches
  ...
  exportPrintableBtn.onclick = async () => {
    try {
      const simHeading = (await customPrompt("Simulink heading text (exact H2)", CONFIG.HEADING_SIMULINK)) || CONFIG.HEADING_SIMULINK;
      ...
      for (const [title, pagesInGroup] of groups.entries()) {
        ...
        for (const p of pagesInGroup) {
          ...
          if (!reqText && p.file?.path) {
            ...
            const block = extractHeadingBlock(md, CONFIG.HEADING_REQ_TEXT);
            if (block) reqText = block;
          }

          if (reqText) {
            lines.push(`#### ${CONFIG.HEADING_REQ_TEXT}`);
            lines.push(reqText);
            lines.push(``);
          }
          ...
          if (p.file?.path) {
            ...
            if (simBlock && simBlock.trim().length > 0) {
              ...
              lines.push(simBlock); // keep embeds like ![[...]] intact
              lines.push(``);
            }
          }
  ...
  exportCanvasBtn.onclick = async () => {
    ...
    if (currentFilteredPages.length > 200) {
      const confirmText = await customPrompt(
        `You are exporting ${currentFilteredPages.length} items. Type 'yes' to proceed:`,
        ""
      );
      if (confirmText !== "yes") return;
    }

    await exportToCanvas(currentFilteredPages);
  };
  ...
  Object.values(filters).forEach(el => {
    ...
    if (el.type !== "checkbox" && !el.multiple) el.addEventListener("keyup", debounced);
  });
